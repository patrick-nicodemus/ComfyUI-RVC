...
class Config:
    ...
    def load_config_json() -> dict:
        ...
        for config_file in version_config_list:
            p = f"{now_dir}/inuse/{config_file}"
            if not os.path.exists(p):
                shutil.copy(f"{now_dir}/{config_file}", p)
            with open(f"{now_dir}/inuse/{config_file}", "r") as f:
    ...
    def use_fp32_config(self):
        for config_file in version_config_list:
            ...
            with open(f"{now_dir}/inuse/{config_file}", "w") as f:
                f.write(strr)
